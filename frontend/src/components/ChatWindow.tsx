...
export default function ChatWindow({
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--bg)" }}>
      ...
      {/* ── Messages ── */}
      {/*
        FIX: The outer div uses overflow-y-auto for scrolling.
        The empty state MUST use min-h-full (not h-full) + justify-center.
        Using h-full + justify-center inside overflow-y-auto causes content to
        overflow upward into negative scroll space, which browsers cannot reach.
        min-h-full lets the container grow if content is tall, keeping it scrollable.
      */}
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-6">

        {/* Empty state */}
        {isEmpty && !loading && (
          <div className="flex flex-col items-center justify-center min-h-full gap-10 py-6 animate-fade-in">
            {/* Hero */}
