        ...
        {tab === "cases" && (
          <>
            <div className="flex items-center justify-between">
              <p className="text-white/60 text-sm">{data?.active_cases?.length ?? 0} active · {s?.total_cases ?? 0} total</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={key} className={key === "title" ? "md:col-span-2" : ""}>
                      ...
                      <input type={type} value={(formData as any)[key]} placeholder={placeholder}
                  ...
                  {[
                    ...
                    <div key={key}>
                      <label className="text-[0.65rem] text-white/50 uppercase tracking-widest mb-1 block">{label}</label>
                      <select value={(formData as any)[key]}
                        onChange={e => setFormData(p => ({ ...p, [key]: e.target.value }))}
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white
            ...
            <div className="space-y-3">
              {(data ? [...(data.active_cases || []), ...(data.upcoming_hearings || [])].filter(
                (c, i, arr) => arr.findIndex(x => x.case_uuid === c.case_uuid) === i
              ) : []).map(c => (
